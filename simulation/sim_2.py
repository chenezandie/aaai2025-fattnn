...
def tenFM_sim(Ft, dims, lambda_val=1, A=None):
    ...
    if A is None:
        ...
        for i, Ai in enumerate(A):
            ...
            X = np.tensordot(Q, X, axes=([1], [0]))
            X = np.moveaxis(X, 0, -1)
    E = np.random.normal(loc=0, scale=1,size=(t, *dims))
...
for seed in range(1,21):
    ...
    with torch.no_grad():
        ...
        # RPE calculation
        Y_mean_obs = torch.mean(Y)
        numerator = torch.norm(predicted - Y_test, p='fro')**2
        #denominator = torch.norm(Y_test - Y_mean_obs, p='fro')**2
        rpe = numerator / denominator
        rpe_values.append(rpe.item())
