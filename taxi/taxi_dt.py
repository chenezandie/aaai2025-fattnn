...
cores = []
factors_list = []
for t in range(X_train_np.shape[0]):
    core, factors = tucker(X_train_np[t], rank=ranks)
    cores.append(core)
    factors_list.append(factors) 
X_train = torch.tensor(cores, dtype=torch.float32)  # Shape: (T, 1, depth, height, width)
