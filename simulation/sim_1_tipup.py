...
import pandas as pd
X_test_flattened = X_test.reshape(X_test.shape[0], -1)  # Flatten each sample but keep the first dimension
Y_test_flattened = Y_test.reshape(Y_test.shape[0], -1)
X_train_flattened = X_train.reshape(X_train.shape[0], -1)  # Flatten each sample but keep the first dimension
Y_train_flattened = Y_train.reshape(Y_train.shape[0], -1)

# Convert to DataFrame
X_test_df = pd.DataFrame(X_test_flattened)
Y_test_df = pd.DataFrame(Y_test_flattened)
X_train_df = pd.DataFrame(X_train_flattened)
Y_train_df = pd.DataFrame(Y_train_flattened)
# Save to CSV
X_test_df.to_csv('X_test_sim1.csv', index=False)
Y_test_df.to_csv('Y_test_sim1.csv', index=False)
X_train_df.to_csv('X_train_sim1.csv', index=False)
Y_train_df.to_csv('Y_train_sim1.csv', index=False)
...
for seed in range(1,21):
    ...
    # Train the model
    train_tcn_model(tcn_model, X_train, Y_train, epochs=300, lr=0.01)
