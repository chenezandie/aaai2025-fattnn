...
#Ft = Ft + np.random.normal(loc=100, scale=100,size=(t, *r))
Y = generate_Y(Ft, dims, R, output_shape, noise_level=1)
...
class LSTMModel5D(nn.Module):
    def __init__(self, input_dims, hidden_dim, layer_dim, output_dims, dropout=0.2):
        super(LSTMModel5D, self).__init__()
        self.hidden_dim = hidden_dim
        self.layer_dim = layer_dim
        self.seq_len = input_dims[-1]
        self.output_dims = output_dims

        lstm_input_size = input_dims[0] * input_dims[1] * input_dims[2]
        self.lstm = nn.LSTM(lstm_input_size, hidden_dim, layer_dim, batch_first=True, dropout=dropout)
        self.fc = nn.Linear(hidden_dim, output_dims[0] * output_dims[1] * output_dims[2])

    def forward(self, x):
        batch_size, channels, dim1, dim2, seq_len = x.size()
        x = x.permute(0, 4, 1, 2, 3).reshape(batch_size, seq_len, -1)
        h0 = torch.zeros(self.layer_dim, x.size(0), self.hidden_dim).requires_grad_()
        c0 = torch.zeros(self.layer_dim, x.size(0), self.hidden_dim).requires_grad_()
        out, (hn, cn) = self.lstm(x, (h0.detach(), c0.detach()))
        out = out[:, -1, :]
        out = self.fc(out)
        out = out.view(batch_size, *self.output_dims)
        return out

# Model hyperparameters
input_dims = (1, 25, 25, 12)  # Example input dimensions excluding seq_len
hidden_dim = 64  # Number of hidden neurons
layer_dim = 3  # Number of hidden layers
output_dims = (6, 8, 6)  # Example output dimensions
dropout = 0.2  # Dropout rate

# Initialize the LSTM model
lstm_model_5d = LSTMModel5D(input_dims, hidden_dim, layer_dim, output_dims, dropout)

# Function to train LSTM model
def train_lstm_model_5d(model, X_train, Y_train, epochs=100, lr=0.001):
    optimizer = Adam(model.parameters(), lr=lr)
    criterion = nn.MSELoss()
    model.train()
    for epoch in range(epochs):
...
# Lists to store MSE and RMSE values for each run
mse_values_lstm = []
rmse_values_lstm = []

# Run for multiple seeds to average results
for seed in range(1, 21):
    torch.manual_seed(seed)
    np.random.seed(seed)
    lstm_model_5d = LSTMModel5D(input_dims, hidden_dim, layer_dim, output_dims, dropout)
    train_lstm_model_5d(lstm_model_5d, X_train, Y_train, epochs=200, lr=0.2)
    lstm_model_5d.eval()

    # No gradient computation
    with torch.no_grad():
        predicted_lstm = lstm_model_5d(X_test)

        # MSE calculation
        mse_loss_lstm = nn.MSELoss()
        mse_lstm = mse_loss_lstm(predicted_lstm, Y_test)
        mse_values_lstm.append(mse_lstm.item())

        # Calculate RMSE
        rmse_lstm = np.sqrt(mse_lstm.item())
        rmse_values_lstm.append(rmse_lstm)

# Calculate average MSE and RMSE
average_mse_lstm = sum(mse_values_lstm) / len(mse_values_lstm)
average_rmse_lstm = sum(rmse_values_lstm) / len(rmse_values_lstm)

print(f"Average Test MSE over runs (LSTM): {average_mse_lstm}")
print(f"Average Test RMSE over runs (LSTM): {average_rmse_lstm}")
