...
specific_ids = [4, 12, 13, 88, 87, 261, 209, 231, 45, 148, 125, 144, 211, 232, 158, 249, 114, 113, 79]#specific_ids = [4, 12, 13, 88, 87, 261, 209, 231, 45, 125, 144, 211, 14, 232, 158, 249, 
...
X_train = X_tensor.transpose(1,2,3,4,0)

modes = [0,1,2,3]
dims = [X_train.shape[1],X_train.shape[2],X_train.shape[3],X_train.shape[4]]
r = [2, 4, 4, 2]
ranks = r
h = 0
...
for i in range(k):
    ...
    M_temp = np.zeros((dd[i], dd[i]))
    print(M_temp.shape)
    x_left = x.reshape(dd[0], dd[1], dd[2], dd[3], n)[:,:,:,:,:n-h].transpose(0, 1, 2, 3, 4)  # Adjusted slicing and reshaping
    x_right = x.reshape(dd[0], dd[1], dd[2], dd[3], n)[:,:,:,:,h:n].transpose(0, 1, 2, 3, 4)  # Adjusted slicing and reshaping
            #print("x_left shape:", x_left.shape)
            #print("x_right shape:", x_right.shape)
    Omega = tensordot(x_left, x_right, permutations[i])
    #print("Omega:", Omega.shape)
...
x_hat = None
    # print('x is:',x[1])
modes = [0, 1, 2, 3]  # Modes to contract on
transposed_Q = [q.T for q in ans_Q]
x_hat = multi_mode_dot(x, transposed_Q, modes)
x_hat = multi_mode_dot(x_hat, ans_Q, modes=[0, 1, 2, 3])
norm_percent = tl.norm(x - x_hat, 2) / tl.norm(x, 2)
...
fnorm_resid[0] = np.linalg.norm(x_tnsr - x_hat) / tnsr_norm
    # Update ans.Q from ans_init
transposed_Q = [q.T for q in ans_Q]
Ft = multi_mode_dot(x_tnsr, transposed_Q, [0, 1, 2, 3])

X_train_np = X_train.transpose(4,0,1,2,3)
cores = []
factors_list = []
for i in range(X_train_np.shape[0]):
    abc = Ft.transpose(4,0,1,2,3)
    core = abc[i]
    cores.append(core)
...
num_train = int(X_tensor.shape[0]*0.7)
X_test = X_tensor[num_train:]
X_train = X_tensor[:num_train]
Y_test = Y_tensor[num_train:]
...
train_tcn_model(tcn_model, X_train, Y_train, epochs=200, lr=0.001)
end_time = time.time()
